...
stack --version
```
After cloning this project, go to the project root and enter the two commands
```
...
Value identifiers and postfix operators start with an alphanumeric character, such as `A`, `4` or `a`.
Examples of prefix and infix operator names are `+`, `&&` and `?is-true`.
Examples of value identifier and postfix operator names are `length`, `Size` and `is-even?`.
...
argument if false. This can be achieved with
```
val _+_\Bool : (Bool, [] -> Bool) -> Bool
let (true, b) => true
let (false, b) => b[]
...
For an example of dependent (explicit) arguments, we will first define
the vector data type as a postfix operator on `Ty`:
```
...
Now we can define a (dependent) function which returns the zero
vector of any length `n`, where the codomain of the function
`Nat Vec(n)` depends on `n`,
```
...
Importing in mini yu works like copying the imported file and pasting it in.
There is, however, detection to avoid importing the same file twice and
get name clashes.
